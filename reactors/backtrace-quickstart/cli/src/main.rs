...
/// }
/// 这个模块的代码是从 L10 的宏展开的问题复制过来的。
/// 主要加了 L312-L315 如下
/// let backtrace = wasmtime::WasmBacktrace::force_capture(&caller);
/// for (i, f) in backtrace.frames().iter().enumerate() {
///     println!("#{i}: {f:?}");
/// }
mod bindings;
...
fn main() -> anyhow::Result<()> {
    ...
    wrap_context_request_id::<_, HasSelf<_>>(&mut linker, |v: &mut MyHost| v)
        .context("wrap context.request-id")?;
...
impl HostContext for MyHost {
    fn request_id(&mut self, _self_: wasmtime::component::Resource<Context>) -> i64 {
        123
    }

    fn drop(&mut self, _rep: wasmtime::component::Resource<Context>) -> wasmtime::Result<()> {
        Ok(())
    }
}
...
use bindings::sammyne::helloworld::types::HostWithStore;

pub fn wrap_context_request_id<T, D>(
...
where
    ...
    T: 'static,
{
    let mut inst = linker.instance("sammyne:helloworld/types@1.0.0")?;
    inst.func_wrap(
        "[method]context.request-id",
        move |mut caller: wasmtime::StoreContextMut<'_, T>,
              (arg0,): (wasmtime::component::Resource<Context>,)| {
            ...
            let r = HostContext::request_id(host, arg0);
            Ok((r,))
        },
    )?;
    Ok(())
